...
class ServiceController extends Controller
{
    ...
    public function fetch(Request $request)
    {
        ...
        return Datatables::of($list)
            ...
            ->addColumn('action', function($row){
                $html = '';
                $html .= '
                    <a href="'.route('admin.service.edit', $row->id).'" class="me-2" data-bs-toggle="tooltip" data-bs-placement="top" title="Update Record">
                        <i class="bi bi-pencil-square fs-4 cursor-pointer text-primary"></i>
                    </a>
                    <a href="'.route('admin.service.delete', $row->id).'" class="me-2 delete-item" data-bs-toggle="tooltip" data-bs-placement="top" title="Delete Record">
                        <i class="bi bi-trash fs-4 cursor-pointer text-danger"></i>
                    </a>
                ';
                return $html;
            })
            ->rawColumns(['logo', 'duration', 'price', 'status', 'action'])
            ->make(true);
    }

    public function add()
    {
        return view('admin.service.add', get_defined_vars());
    }
    ...
    public function save(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $service = Service::create($request->except('logo'));
            if (isset($request->logo)) {
                $fileName = Str::random(36).'.'.$request->logo->getClientOriginalExtension();
                $filePath = "services/{$service->id}/{$fileName}";
            ...
            DB::commit();
            return redirect()->route('admin.service.index')->with('Record added successfully!');
        } catch (\Exception $e) {
            DB::rollback();
    ...
    public function update(Request $request, Service $service)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('admin.service.index')->with('Record updated successfully!');
        } catch (\Exception $e) {
            DB::rollback();
            return redirect()->back()->with('error', $e->getMessage());
        }
    ...
    {
        $service->delete();
        return redirect()->route('admin.service.index')->with('Record deleted successfully!');
    }
}
